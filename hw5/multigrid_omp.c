...
double compute_norm(double *u, int N)
{
    ...
    #pragma omp parallel for default(none) shared (u,N) private (i, tmp) reduction(+:norm)
...
void coarsen(double *uf, double *uc, int N) {
    int ic;
    #pragma omp parallel for default(none) shared (uc, uf, N) private (ic)
    for (ic = 1; ic < N/2; ++ic)
...
void refine_and_add(double *u, double *uf, int N)
{
    int i;
    uf[1] += 0.5 * (u[0] + u[1]);
    #pragma omp parallel for default(none) shared (u, uf, N) private (i)
    for (i = 1; i < N; ++i) {
        uf[2*i] += u[i];
...
{
    int i;
    #pragma omp parallel for default(none) shared (u, rhs, N, res, invhsq) private (i)
    for (i = 1; i < N; i++)
...
void jacobi(double *u, double *rhs, int N, double hsq, int ssteps)
{
    ...
    double *unew = calloc(sizeof(double), N+1);
     //#pragma omp parallel for default(none) shared (u, unew, N, omega, hsq, rhs, ssteps) private (i, j)
    for (j = 0; j < ssteps; ++j) {
    #pragma omp parallel for default(none) shared (u, unew, N, omega, hsq, rhs) private (i)
        for (i = 1; i < N; i++){
...
# pragma omp parallel
    ...
    /* rhs on finest mesh */
   #pragma omp parallel for default(none) shared(N, rhs) private (i)
    for (i = 0; i <= N[0]; ++i) {
        rhs[0][i] = 1.0;
    }
